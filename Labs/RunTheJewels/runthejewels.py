...
# Declare penalty for invalid move
PENALTY = 5
HIGH_SCORE_REACHED = False


def main():
    global FPSCLOCK, DISPLAYSURF, GEMIMAGES, GAMESOUNDS, BASICFONT, BOARDRECTS, PENALTY
    ...
    GAMESOUNDS['celebration'] = pygame.mixer.Sound('sounds/celebration.wav')
    GAMESOUNDS['match'] = []
    for i in range(NUMMATCHSOUNDS):
        GAMESOUNDS['match'].append(
            pygame.mixer.Sound('sounds/match%s.wav' % i))
...
def gethighScore():
    """Retrieve high score"""
    return int(open("highscore.txt", "r").readline())


def saveHighScore(high_score):
    """Save high score"""
    open("highscore.txt", "w").write(str(high_score))


def runGame():
    # Plays through a single game. When the game is over, this function returns.
    global HIGH_SCORE_REACHED
    ...
    while True:  # main game loop
        ...
        if clickedSpace and not firstSelectedGem:
            ...
            if matchedGems == []:
                ...
                while matchedGems != []:
                    ...
                    score += scoreAdd
                    # Update high score only when score is greater
                    if score > high_score:
                        ...
                        # Play sound for reaching new high_score
                        GAMESOUNDS['celebration'].play()
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                ...
                # When high score is reached, update the game over text
                if HIGH_SCORE_REACHED:
                    clickContinueTextSurf = BASICFONT.render(f'High Score Reached!: %s ' % (high_score), 1,
...
def drawMovingGem(gem, progress):
    ...
    r = pygame.Rect((pixelx + movex, pixely + movey,
...
def pullDownAllGems(board):
    ...
    for x in range(BOARDWIDTH):
        ...
        board[x] = ([EMPTY_SPACE] * (BOARDHEIGHT -
                                     len(gemsInColumn))) + gemsInColumn
...
def animateMovingGems(board, gems, pointsText, score):
    ...
    while progress < 100:  # animation loop
        ...
        for pointText in pointsText:
            pointsSurf = BASICFONT.render(
                str(pointText['points']), 1, SCORECOLOR)
    ...
    while dropSlots != [[]] * BOARDWIDTH:
        ...
        for x in range(len(dropSlots)):
            if len(dropSlots[x]) != 0:
                ...
                movingGems.append(
                    {'imageNum': dropSlots[x][0], 'x': x, 'y': ROWABOVEBOARD, 'direction': DOWN})
...
def drawHighScore(score):
    ...
    scoreRect.bottomright = (590, WINDOWHEIGHT - 6)
